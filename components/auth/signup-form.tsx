...
export function SignupForm({ email: initialEmail, nextPath }: SignupFormProps) {
	...
	async function onSubmit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const payload = (await response.json()) as {
				ok: boolean
				error?: string
				data?: { accessStatus?: string; needsVerification?: boolean }
			}
			...
			if (payload.data?.needsVerification) {
				...
				window.location.assign(`/verify?${params}`)
				return
			}
